...
#Seedling and sapling count models
allsdtwide$plot2<-as.factor(allsdtwide$plot2)
allsptwide$plot2<-as.factor(allsptwide$plot2)
#write.csv(allsptwide,"sapdat.csv",row.names=FALSE)
#write.csv(allsdtwide,"sdlgdat.csv",row.names=FALSE)

#table of counts
sd2020mmod<-glmer(CT2020~TREAT*OLD.CAT+ (1|plot2), family="poisson",data =allsdtwide)


sp2020mmod<-glmer(CT2020~TREAT*OLD.CAT+ (1|plot2), family="poisson",data =allsptwide)
