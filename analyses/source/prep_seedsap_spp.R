...
#check dim (chould be 60 plots * 4 subplot s = 240 rows)
#dim(tshespl)
#dim(tshesdl)
#need to add in missing plots with 0s
...
#sites2007<-sites
sites$YR<-2020
tshesddat<-left_join(sites,tshesdl, by=c("plot2","BASIN","SUBPLOT","YR"))
#table(tshesddat$plot2,tshesddat$YR)#looks good!
tshespdat<-left_join(sites,tshespl, by=c("plot2","BASIN","SUBPLOT","YR"))
...
sdlmerge<-left_join(sites,allspsdl, by=c("plot2","BASIN","SUBPLOT","YR"))
splmerge<-left_join(sites,allspspl, by=c("plot2","BASIN","SUBPLOT","YR"))
...
colnames(allsdtwide)[7]<-colnames(allsptwide)[7]<-"CT2020"
